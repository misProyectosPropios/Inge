...
extender-inclusion : {Γ Δ : Ctx} {A : Form}
                   → Inclusion Γ Δ
                   → Inclusion (Γ , A) (Δ , A)
extender-inclusion {Γ} {Δ} {A} incl {A₁} zero    = zero
extender-inclusion {Γ} {Δ} {A} incl {A₁} (suc b) = suc (incl b)
...
debilitamiento : {Γ Δ : Ctx} {A : Form}
               ...
               → Γ ⊢ A
               → Δ ⊢ A
debilitamiento {Γ} {Δ} incl (AX x)      = AX (incl x) 
debilitamiento {Γ} {Δ} {A} incl (FALSE-e p) = let x = debilitamiento incl p
                                              in FALSE-e {Δ} {A} x
debilitamiento {Γ} {Δ} {A} incl (IMP-i {B} {C} p) = let y = extender-inclusion {Γ} {Δ} {C}  incl
                                                        x = debilitamiento  y  p
                                                    in IMP-i x
debilitamiento {Γ} {Δ}{A} incl (IMP-e p q) = let x = debilitamiento {Γ} {Δ} incl p
                                                 y = debilitamiento {Γ} {Δ} incl q
                                             in IMP-e x y
debilitamiento {Γ} {Δ} {A} incl (DNEG p) = let x = debilitamiento {Γ} {Δ} incl p
                                           in DNEG x
...
lema1 : {Γ : Ctx} {A B C : Form} → Inclusion (Γ , C) (Γ , B , C)
lema1 {Γ} {A} {B} {C} zero    = zero
lema1 {Γ} {A} {B} {C} (suc b) = suc (suc b)

wk1 : {Γ : Ctx} {A B C : Form}
    → Γ , C ⊢ A
    → Γ , B , C ⊢ A
wk1 {Γ} {A} {B} {C} = debilitamiento  (lema1 {Γ} {A} {B} {C})
...
subst : {Γ : Ctx} {A B : Form}
      ...
      → Γ ⊢ A
      → Γ ⊢ B
subst {Γ} {A} {B} p q = let x = IMP-i {Γ} {A} {B} p
                            y = IMP-e {Γ} {A} {B} x
                            z = y q
                         in z
...
reductio-ad-absurdum : {Γ : Ctx} {A : Form}
                     → Γ , NOT A ⊢ FALSE
                     → Γ ⊢ A
reductio-ad-absurdum {Γ} {A} p = DNEG
                                   (IMP-i p)
...
consequentia-mirabilis : {Γ : Ctx} {A : Form}
                       → Γ , NOT A ⊢ A
                       → Γ ⊢ A
consequentia-mirabilis {Γ} {A} p = DNEG
                                      (IMP-i
                                          (IMP-e {Γ , (IMP A FALSE)} {A}
                                              (AX {Γ , (IMP A FALSE)} zero)
                                              p))
...
AND-i : {Γ : Ctx} {A B : Form}
      ...
      → Γ ⊢ B
      → Γ ⊢ AND A B
AND-i {Γ} {A} {B} p q = let y = IMP A (NOT B)
                            x = Γ , y -- , IMP --
                        in IMP-i (NOT-e {x} {B} (
                                     IMP-e {x} {A}
                                         (AX zero)
...
AND-e1 : {Γ : Ctx} {A B : Form}
       → Γ ⊢ AND A B
       → Γ ⊢ A
AND-e1 {Γ} {A} {B} p = let y = IMP A FALSE
                           x = Γ , y
                     in DNEG
                          (NOT-i
                             (IMP-e {x} {IMP A (NOT B)} {FALSE}
                                 (wk {Γ} p)
                                 (IMP-i
                                   (NOT-i
                                      (IMP-e {x , A , B } {A} 
                                         (AX (suc (suc zero)))
                                         (AX (suc zero)))))))  -- Recordar que es posible razonar clásicamente usando DNEG.
...
AND-e2 {Γ} {A} {B} p = DNEG
                          (NOT-i
                             (IMP-e {Γ , IMP B FALSE} {IMP A (NOT B)} 
                                (wk p)
                                (IMP-i
                                   (NOT-i
                                      (IMP-e {Γ , IMP B FALSE , A , B} {B}
                                         (wk (wk (AX zero)))
                                         (AX zero))))))
...
OR-e : {Γ : Ctx} {A B C : Form}
     ...
     → Γ , B ⊢ C
     → Γ ⊢ C
OR-e {Γ} {A} {B} {C} p q r = consequentia-mirabilis (
                              subst
                                 (wk1 q)
                                 (reductio-ad-absurdum
                                   (NOT-e {Γ , NOT C , NOT A} {B}
                                     (NOT-i
                                        (NOT-e {Γ , NOT C , NOT A , B} {C}
                                           (AX (suc (suc zero)))
                                           (wk1 (wk1 r))))
                                     (IMP-e { Γ , NOT C , NOT A} {NOT A}
                                        (wk1 (wk p))
                                        (AX zero))) )) -- {Γ , (NOT C) , (NOT A)}
...
deduccion-natural-correcta (AX x)                     = {!!}
deduccion-natural-correcta (FALSE-e secuente)         = {!!}
deduccion-natural-correcta (IMP-i secuente)           = {!!}
deduccion-natural-correcta (IMP-e secuente secuente₁) = {!!}
deduccion-natural-correcta (DNEG secuente)            = {!!}
